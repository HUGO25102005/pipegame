...
FONDO_IMAGEN = pygame.image.load("images/mary.jpeg")  # Reemplaza "ruta_de_la_imagen.jpg" con la ruta de tu imagen
...
fuente_indicacion = pygame.font.Font(None, 1000)
...
while not terminado:
    ...
    FONDO_COLOR = (255, 255, 255)  # Define the background color (e.g., white)
        ...
        for y in range(min(y_inicio, y_fin), max(y_inicio, y_fin) + 1):
            for x in range(min(x_inicio, x_fin), max(x_inicio, x_fin) + 1):
                if 0 <= y < len(MATRIZ_LETRAS) and 0 <= x < len(MATRIZ_LETRAS[y]):
                   ...
                   # ventana.blit(texto, (15 + x * TAMANO_CELDA, y * TAMANO_CELDA)) este es un error y lo escribi como comentario para darme cuenta de porque no abria la ventana
